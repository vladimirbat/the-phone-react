...
import { RouterController } from "../domain/adapters/router-controller";
import { ViewsRegistry } from "../domain/adapters/views-registry";
...
export class WebRouterController implements RouterController {
  ...
  private getNavigationPath(viewName: ViewsRegistry, params: KeyValueObject<string>[]): string{
    ...
    return routesRegistry[viewName] + params.map((param) => {
      let path = '';
